...
export async function fetchBookContent(bookId: string) {
  ...
  try {
    ...
    const response = await authApi.get(`/reader/content/${bookId}`, {
    ...
    console.log(
      '[BookContent API] ✓ Successfully fetched blob for bookId:',
      bookId,
      'size:',
      response.data.size,
      'bytes',
    ...
    if ((error as AxiosError).response?.status === 401) {
      ...
      throw new Error(
        'Request timeout. The book file is too large or connection is slow.',
      )
    } else {
      throw new Error(
        `Failed to fetch book content: ${(error as AxiosError).message}`,
      )
    }
  }
}
